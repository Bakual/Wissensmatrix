		...
		if ($this->params->get('show_empty_categories') or $child->getNumItems(true) or count($child->getChildren())) :
			...
			<div<?php echo $class; ?>>
				...
				<h3 class="page-header item-title">
					<a href="<?php echo JRoute::_(WissensmatrixHelperRoute::getWorkersRoute($child->id));?>">
						<?php echo $this->escape($child->title); ?>
					</a>
					<?php if ($this->params->get('show_cat_num_items', 1)) : ?>
						<span class="badge badge-info tip hasTooltip" title="<?php echo JText::_('COM_WISSENSMATRIX_NUM_ITEMS'); ?>">
				...
				<?php if ($this->params->get('show_subcat_desc') == 1) :
					if ($child->description) : ?>
						<div class="category-desc">
							<?php echo JHtml::_('content.prepare', $child->description, '', 'com_wissensmatrix.category'); ?>
						</div>
					<?php endif;
				endif;
				if (count($child->getChildren()) > 0) : ?>
