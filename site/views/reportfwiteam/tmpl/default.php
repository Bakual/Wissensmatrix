				...
				<div id="filter-bar" class="filters btn-toolbar">
					<?php if ($this->params->get('filter_field')) : ?>
						...
						<div class="btn-group filter-select input-append">
							...
							<a href="<?php echo JRoute::_('index.php?view=reportfwiteam&id='.$this->item->id.'&teamid='.$this->parent->id); ?>" class="btn addon" title="<?php JText::printf('COM_WISSENSMATRIX_GET_PARENT_TEAM', $this->parent->title); ?>"><i class="icon-arrow-up"></i></a>
						</div>
					<?php endif; ?>
				</div>
			<?php endif; ?>
					...
					<tbody>
						<?php foreach ($this->workers as $worker) :
							...
							<tr>
								...
								<td>
									<a href="<?php echo JRoute::_('index.php?view=reportfwiteam&id='.$this->item->id.'&teamid='.$worker->catid); ?>"><?php echo $worker->category_title; ?></a>
								</td>
								<td class="center">
