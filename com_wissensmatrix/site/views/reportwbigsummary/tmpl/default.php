...
?>
<div class="category-list<?php echo $this->pageclass_sfx;?> wm-reportwbigsummary-container<?php echo $this->pageclass_sfx; ?>">
				...
				<div id="filter-bar" class="filters btn-toolbar">
					<?php if ($this->params->get('filter_field')) : ?>
						<div class="btn-group filter-select input-append">
							...
							<?php if ($this->parent) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_wissensmatrix&view=reportwbigsummary&teamid='.$this->parent->id); ?>" class="btn addon" title="<?php JText::printf('COM_WISSENSMATRIX_GET_PARENT_TEAM', $this->parent->title); ?>" rel="tooltip"><i class="icon-arrow-up"></i></a>
							<?php endif; ?>
						</div>
			...
			<?php else : ?>
				<?php foreach ($this->items as $item) : 
					...
					<h3 class="page-header">
						...
						<?php if ($count) : ?>
							<span class="badge badge-info" title="<?php echo JText::_('COM_WISSENSMATRIX_NUM_ITEMS'); ?>" rel="tooltip">
								<?php echo $count; ?>
							</span>
							<button type="button" data-toggle="collapse" data-target="#wbig<?php echo $item->id; ?>" class="btn btn-mini pull-right">
								<span class="icon-plus"></span>
							</button>
						<?php endif; ?>
					</h3>
						...
						<?php else : ?>
							<?php foreach ($wbis as $wbi) : ?>
								<h4 class="page-header">
									...
									<span class="badge" title="<?php echo JText::_('COM_WISSENSMATRIX_NUM_ITEMS'); ?>" rel="tooltip"><?php echo $wbi->mit_count; ?></span>
									<?php if ($wbi->refresh) : ?>
										 <span class="label label-small pull-right"><?php echo JText::sprintf('COM_WISSENSMATRIX_REFRESH_TEXT', $wbi->refresh); ?></span>
									<?php endif; ?>
								</h4>
							<?php endforeach; ?>
						<?php endif; ?>
