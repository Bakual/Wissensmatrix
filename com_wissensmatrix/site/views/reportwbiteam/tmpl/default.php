	...
	<div class="cat-items">
		<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" id="adminForm"
				...
				<div id="filter-bar" class="filters btn-toolbar">
					<?php if ($this->params->get('filter_field')) : ?>
						<div class="btn-group filter-select input-append">
							...
							<?php if ($this->parent) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_wissensmatrix&view=reportwbiteam&id=' . $this->item->id . '&teamid=' . $this->parent->id); ?>"
								   class="btn addon"
			...
			<h3>
				...
				<small><?php echo JText::_('COM_WISSENSMATRIX_WBIG'); ?>: <a
						href="<?php echo JRoute::_('index.php?option=com_wissensmatrix&view=reportwbigteam&id=' . $this->item->wbig_id); ?>"><?php echo $this->item->wbig_title; ?></a>
				</small>
			</h3>
			<?php if ($this->item->refresh) : ?>
				<div
					class="well well-small"><?php echo JText::sprintf('COM_WISSENSMATRIX_REFRESH_TEXT', $this->item->refresh); ?></div>
			<?php endif; ?>
